...
namespace Drupal\blp_flex_groups_project\Subscriber;
  ...
  protected function alterRoutes(RouteCollection $collection) {
    ...
    foreach ($join_routes as $name => $argument) {
      if ($route = $collection->get($name)) {
        ...
        $requirements = array_merge($current, ['_project_group_join_permission' => $argument]);
    ...
    \Drupal::moduleHandler()
      ->alter('blp_flex_groups_project_content_routes', $content_routes);

    foreach ($content_routes as $name) {
      if ($route = $collection->get($name)) {
        $current = $route->getRequirements();
        $requirements = array_merge($current, ['_project_group_content_visibility' => 'public']);
        $route->addRequirements($requirements);
      }
    }
  }
